# v1.1.2 - Stability & Debugging Release
...
### Toolbar Window
- Fixed window height to 100px to accommodate native titlebar (was 64px - buttons were clipped)
- Window now uses `decorations: false` for better appearance and button accessibility
- Module-level invoke loading with logging for debugging startup issues

### Sidebar Version
- Fixed hardcoded `v0.1.0` display - now shows `v1.1.2`

### Comprehensive Logging
- App.tsx: startup, plugin registration, page navigation, plugin toggling, floating window open/close, theme changes
- FloatingApp: component loading success/failure with pluginId context
- PythonOCRConfig: mount, launch/stop attempts with full error messages
- FloatingToolbar: mount, button clicks, invoke availability
- PythonPluginBase: activate/deactivate lifecycle with script path resolution
- All errors now log full error objects for debugging
...
### Floating Toolbar
- Fixed empty content in always-on-top toolbar window - buttons now render correctly
- Close button (X) and Escape key now properly close the toolbar
- Simplified internal implementation with module-level invoke loading

### Debug Logger
- Logger now captures console output from app startup (imported in main.tsx)
- Added startup log entry for visibility

### Python OCR Plugin
- Python scripts are now bundled as Tauri resources - plugin can find and launch them in production builds

### Sidebar Navigation
- Added "All Plugins" button to sidebar to navigate back from plugin config pages
